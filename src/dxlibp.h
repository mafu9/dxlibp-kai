...
/* Data save resources */
#define DXP_SAVE_RESOURCE_ICON1	0
#define DXP_SAVE_RESOURCE_ICON0	1
#define DXP_SAVE_RESOURCE_PIC1	2
#define DXP_SAVE_RESOURCE_SND0	3

/* Data save modes */
#define DXP_SAVE_MODE_SAVE		5
#define DXP_SAVE_MODE_AUTOSAVE	1
#define DXP_SAVE_MODE_LOAD		4
#define DXP_SAVE_MODE_DELETE	6

/* Image save type */
#define DXP_SAVE_TYPE_JPEG	1
#define DXP_SAVE_TYPE_PNG	2
#define DXP_SAVE_TYPE_BMP	3

/* Jpeg */
#define DXP_SAVE_JPEG_QUALITY_MAX	100
#define DXP_SAVE_JPEG_PROGRESS		1
...
//独断と偏見で選んだ色
#define DXP_COLOR_CLEARWHITE	0x7FFFFFFF
#define DXP_COLOR_CLEARBLACK	0x7F000000

#define DXP_COLOR_WHITE			0xFFFFFFFF
#define DXP_COLOR_WHITESMOKE	0xFFF5F5F5
#define DXP_COLOR_LITEGRAY		0xFFD3D3D3
#define DXP_COLOR_SILVER		0xFFC0C0C0
#define DXP_COLOR_GRAY			0xFF808080
#define DXP_COLOR_DARKGRAY		0xFF3F3F3F
#define DXP_COLOR_BLACK			0xFF000000

#define DXP_COLOR_GREEN			0xFF008000
#define DXP_COLOR_DARKGREEN		0xFF006400
#define DXP_COLOR_LIME			0xFF00FF00
#define DXP_COLOR_GREENYELLOW	0xFF2FFFAD

#define DXP_COLOR_YELLOW		0xFF00FFFF
#define DXP_COLOR_LEMON			0xFF80FFFF
#define DXP_COLOR_YELLOGREEN	0xFF00FF80

#define DXP_COLOR_BLUE			0xFFFF0000
#define DXP_COLOR_AQUA			0xFFFFFF00
#define DXP_COLOR_SKYBLUE		0xFFEBCE87
#define DXP_COLOR_ROYALBLUE		0xFFE16941
#define DXP_COLOR_LIGHTBLUE		0xFFFA0000
#define DXP_COLOR_DARKBLUE		0xFF8B0000

#define DXP_COLOR_PURPLE		0xFF800080
#define DXP_COLOR_PLUM			0xFFDDA0DD

#define DXP_COLOR_RED 			0xFF0000FF
#define DXP_COLOR_TOMATO		0xFF4763FF
#define DXP_COLOR_DARKRED		0xFF00008B
#define DXP_COLOR_ORANGE		0xFF00A5FF
#define DXP_COLOR_PEARLORANGE	0xFF75BAFF
#define DXP_COLOR_PINK			0xFFCDC0FF

#define DXP_COLOR_BROWN			0xFF2A2AA5
#define DXP_COLOR_CHOCOLATE		0xFF1E69D2
#define DXP_COLOR_GOLD			0xFF00D7FF
 ...
 * 
 * サイズが512x512までのPNG、JPEG、BMP画像を読み込むことが出来ます
...
/**
 * メモリ上の画像を読み込んで画面に表示する(本家互換)
 * 
 * サイズが512x512までのPNG、JPEG、BMP画像を読み込むことが出来ます
 * @param x ロードした画像を描画する矩形の左上頂点のX座標
 * @param y ロードした画像を描画する矩形の左上頂点のY座標
 * @param data メモリ上の画像データバッファ
 * @param size バッファサイズ
 * @param trans 透過色を入れるか、のフラグ。TRUEで透過色有効になる
 * @retval 0 成功
 * @retval -1 失敗
...
/*@{*/
/**
 * ダイアログ系の関数の背景描写用のコールバックを設定します。
 *
 * @param func 背景描写用のコールバック関数（引数void, 返値voidの関数）を指定
 * NULLを指定するとデフォルトに戻ります。
 *
 * @return 常に0
*/
...
/**
 * 公式ダイアログ画面を利用してセーブ、ロードする
 *
 * LOADを指定した場合はbufに復号化したセーブデータが入ります
 *
 * modeに指定可能な値
 * - DXP_SAVE_MODE_SAVE	
 * - DXP_SAVE_MODE_AUTOSAVE
 * - DXP_SAVE_MODE_LOAD
 * - DXP_SAVE_MODE_DELETE
 *
 * @param mode セーブダイアログのモード
 * @param buf セーブするためのデータ
 * @param size bufのサイズ
 * @param GameName セーブデータのフォルダ名
 * @param GameTitle ゲームのタイトル
 * @param SaveTitle セーブデータのタイトル
 * @param SaveDetail セーブデータの詳細
 *
 *  - LOAD モードの時
 * @retval 0 成功
 * @retval -1 セットアップに失敗
 * @retval -2 キャンセル
 * @retval -3 読み込めるデータがなかった
 *
 *  - AUTOSAVE, SAVE, DELETE モードの時
 * @retval 0 成功
 * @retval -1 失敗
*/
int ShowSaveDialog(
 ...
 * - DXP_SAVE_RESOURCE_PIC1
 * - DXP_SAVE_RESOURCE_ICON0
 * - DXP_SAVE_RESOURCE_SND0
 *
 * @param type 設定するリソースの種類
 * @param buf リソースバッファ
 * @param size リソースバッファのサイズ
 *
 * @retval 0 成功
 * @retval -1 失敗
*/
...
/**
 * 画面をキャプチャして保存する
 * 保存するファイル名と同じファイルがある場合、元のファイルに上書きされます
 * 
 * typeに指定可能な値
 * - DXP_SAVE_TYPE_JPEG
 * - DXP_SAVE_TYPE_PNG
 ...
 * JPEG保存時の設定を変更する
 *
 * @param quality 1～100までの数値
 * @param pro TRUE(1)ならプログレッシブ画像
 *
 * @return 0 成功
 * @retval -1 失敗
*/
...
/**
 * デバッグ用というか、DXPで機能拡張するのには限界があるので
 *
 * @param dest バッファ
 *
 * @retval 0 成功
...
int GetSoundCurrentFrame(int handle);

/* Convert*/

//SJISからUTF8へ
inline unsigned int SJIS_to_UTF8(void* dest, const void *src);

//SJISからUTF16へ
inline unsigned int SJIS_to_UTF16(void* dest, const void *src);

//UTF16からSJISへ
inline unsigned int UTF16_to_SJIS(void* dest, const void *src);
